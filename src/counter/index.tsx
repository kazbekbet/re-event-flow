import { useStore } from '../../lib/main.ts';
...
function EvenTransformIndicator() {
  ...
  return (
    <h1>
      Is counter even: <span style={{ color: isEven ? 'green' : 'red' }}>{isEven ? 'Yes' : 'No'}</span>
    </h1>
  );
}
...
function Performance() {
  ...
  return <>{Boolean(performance) && <p>1 million increments performs on: {performance} ms.</p>}</>;
}
...
function Buttons() {
  return (
    <>
      ...
      <button onClick={model.handleAsyncIncrementSuccess}>Async increment</button>
      <button onClick={model.handleAsyncIncrementError}>Async error</button>
      <button onClick={model.incrementMillionTimes}>Increment 1M times</button>
