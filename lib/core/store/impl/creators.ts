...
async function promisifyFn<Args, Payload>(asyncFn: SetAsyncEvent.ArgFn<Args, Payload>, ...args: Args[]) {
...
export function setComputedStore<OriginalStoreVal, ComputedStoreVal = OriginalStoreVal>({
  store,
  condition,
  transform,
  ...
  function transformValue(val: OriginalStoreVal) {
    return typeof transform === 'function' ? transform(val) : (val as unknown as ComputedStoreVal);
  }
  ...
  return new Store(transformValue(store.getState()), {}).on(updateEvent, (_, payload) => transformValue(payload));
}
